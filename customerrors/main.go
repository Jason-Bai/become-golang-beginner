...
func circleArea(radius float64) (float64, error) {
	if radius < 0 {
		...
		return 0, fmt.Errorf("Area caculation failed, radius %.2f is less than zero", radius)
	}
...
func main() {
	fmt.Println("1. custom error")
	customError()
	fmt.Println("2. custom error2")
	customError2()
	fmt.Println("3. more error info")
	moreErrorInfo()
}
